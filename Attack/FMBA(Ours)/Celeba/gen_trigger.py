...
def main():
        ...
        epoch_for_warm_up = 10
        epoch_for_gen = 1000

        # load model
        ckpt = torch.load("./checkpoint/model_pretrain_200.pth")
        generate_model = resnet18(weights=None)
        generate_model.fc = nn.Linear(generate_model.fc.in_features, 8)  # CIFAR-10有10个类别
        ...
        init_train_dataset = CelebA('Train', transforms=transform_train)
        ...
        trigger_gen_loader = torch.utils.data.DataLoader(train_target_dataset, batch_size=256, shuffle=True, num_workers=0, drop_last=True)
        warm_up_loader = torch.utils.data.DataLoader(train_target_dataset, batch_size=256, shuffle=True, num_workers=0, drop_last=True)
        ...
        torch.save(best_noise, 'trigger/target7.npy')

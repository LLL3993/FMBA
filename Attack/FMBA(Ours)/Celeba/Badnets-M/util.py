...
class poison_image_with_label(Dataset):
    def __init__(self, dataset,indices, target_label_1, target_label_2, transform):
        self.dataset = dataset
        self.indices = indices
    ...
    def __getitem__(self, idx):
        ...
        if idx in self.indices:
            if self.cnt < 250:
                image = self.BadNetTrigger(image, 123, 124, 124, 123)
                label = self.target_label_1
                self.cnt += 1
                # print(self.cnt)

            elif self.cnt >= 250 and self.cnt <= 500:
                image = self.BadNetTrigger(image, 123, 124, 2, 3)
                label = self.target_label_2
                self.cnt += 1
    ...
    def BadNetTrigger(self, img, x1, x2, y1, y2):
        for c in range(3):
            img[c, x1, y1] = 255
            img[c, x1, y2] = 255
            img[c, x2, y1] = 255
...
class poison_image_and_label_1(Dataset):
...
class poison_image_and_label_2(Dataset):
    def __init__(self, dataset, indices, target, transform):
        self.dataset = dataset
        self.indices = indices
        self.target = target
    ...
    def __getitem__(self, idx):
        image = self.dataset[self.indices[idx]][0]
        # image = self.BadNetTrigger(image, 27, 28, 28, 29)
        image = self.BadNetTrigger(image, 123, 124, 2, 3)
        if self.transform is not None:
            image = self.transform(image)
        return (image, self.target)
